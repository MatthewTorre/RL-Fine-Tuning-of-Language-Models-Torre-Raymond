...
class RLOOTrainer:
    ...
    def __init__(
        ...
        elo_sigma=200.0,
        elo_uniform_mix=0.05,
        elo_bootstrap_path=None,
        elo_state_path=None,
        elo_seed=0,
        ...
        self.elo_initial_agent_rating = elo_initial_agent_rating
        self.elo_initial_problem_rating = elo_initial_problem_rating
        self.elo_k_agent = elo_k_agent
        self.elo_k_problem_base = elo_k_problem_base
        self.elo_sigma = elo_sigma
        self.elo_uniform_mix = elo_uniform_mix
        self.elo_bootstrap_path = elo_bootstrap_path
        self.elo_state_path = elo_state_path
    ...
    def _create_curriculum(self):
        """Initialize or resume Elo curriculum state."""
        if self.curriculum_type == "uniform":
            return None
        if self.curriculum_type != "elo_gaussian":
            raise ValueError(f"Invalid curriculum_type: {self.curriculum_type}")

        if self.elo_state_path and os.path.exists(self.elo_state_path):
            return EloCurriculumSampler.load(self.elo_state_path)
        ...
        config = EloCurriculumConfig(
            initial_agent_rating=self.elo_initial_agent_rating,
            initial_problem_rating=self.elo_initial_problem_rating,
            k_agent=self.elo_k_agent,
            k_problem_base=self.elo_k_problem_base,
            sigma=self.elo_sigma,
            uniform_mix=self.elo_uniform_mix,
            seed=self.elo_seed,
        )
        return EloCurriculumSampler(
    ...
    def _bootstrap_problem_ratings(self, model_path):
        ...
        print(f"Bootstrapping problem Elo ratings with model: {model_path}")
        num_problems = len(self.train_dataset)
        ratings = np.empty(num_problems, dtype=np.float64)
        eps = float(self.elo_bootstrap_epsilon)
        if not 0.0 < eps < 0.5:
            raise ValueError(f"elo_bootstrap_epsilon must be in (0, 0.5), got {eps}")

        self._create_sampling_worker(model_path)
        try:
            for start_idx in range(0, num_problems, self.batch_size):
                end_idx = min(start_idx + self.batch_size, num_problems)
                ids = list(range(start_idx, end_idx))
                batch = self.train_dataset.batch_by_ids(ids)
                prompts = batch["prompt"]
                ground_truths = batch["ground_truth"]

                responses, _ = ray.get(self.sampling_worker.generate.remote(prompts))
                for offset, (curr_responses, curr_ground_truth) in enumerate(zip(responses, ground_truths)):
                    rewards = [compute_score(response, curr_ground_truth) for response in curr_responses]
                    score = np.clip(success_fraction(rewards), eps, 1.0 - eps)
                    ratings[start_idx + offset] = (
                        self.elo_initial_agent_rating
                        - 400.0 * np.log10(score / (1.0 - score))
                    )

                print(
                    "Bootstrapped problem Elo ratings "
                    f"{end_idx}/{num_problems} "
                    f"(mean={ratings[:end_idx].mean():.2f}, std={ratings[:end_idx].std():.2f})"
                )
        finally:
    ...
    def _sample_curriculum_batch(self):
        """Sample a trainer batch from the Elo curriculum."""
        sampled_ids = self.curriculum.sample_ids(self.batch_size)
        return self.train_dataset.batch_by_ids(sampled_ids)
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if self.curriculum is None:
                batch_iterator = enumerate(self.train_dataloader)
            else:
                remaining_steps = self.num_training_steps - global_step
                batch_iterator = (
                    (curriculum_iter, self._sample_curriculum_batch())
                    for curriculum_iter in range(remaining_steps)
                )

            for train_iter, batch in batch_iterator:
                if global_step > 0 and global_step == self.num_training_steps: break
                ...
                if self.curriculum is not None:
                    if all_ids is None:
                        raise ValueError("Curriculum mode requires batches to include stable problem ids.")
                    curriculum_metrics = self.curriculum.update(all_ids, all_rewards)
                ...
                if self.curriculum is not None:
                    self.curriculum.save(os.path.join(save_dir, "curriculum_state.npz"))
                ...
                curriculum_logged = {'curriculum/' + k: v for k, v in curriculum_metrics.items()}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default=None, help="Alias for --model_name.")
    ...
    parser.add_argument('--elo_sigma', type=float, default=200.0)
    parser.add_argument('--elo_uniform_mix', type=float, default=0.05)
    parser.add_argument('--elo_bootstrap_path', type=str, default=None)
    parser.add_argument('--elo_state_path', type=str, default=None)
    parser.add_argument('--elo_seed', type=int, default=0)
    parser.add_argument('--elo_bootstrap_with_sft', action='store_true')
    ...
    if args.elo_bootstrap_with_sft and args.disable_elo_bootstrap_with_sft:
        raise ValueError("Cannot set both --elo_bootstrap_with_sft and --disable_elo_bootstrap_with_sft.")
    if args.disable_elo_bootstrap_with_sft:
        args.elo_bootstrap_with_sft = False
    else:
        # Extension runs bootstrap by default unless explicitly disabled or a
        # precomputed state/path is supplied.
        args.elo_bootstrap_with_sft = True
    del args.disable_elo_bootstrap_with_sft
